...
function convertGradeToScale(grade) {
    if (grade >= 85) {
        return 4.0;
    } else if (grade >= 70) {
        return 3.0;
...
function addCourse() {
    ...
    const gradeScale = convertGradeToScale(grade);
    const points = credits * gradeScale;

    courses.push({ subject, credits, grade, gradeScale, points });
...
function calculateGPA() {
    ...
    if (totalCredits === 0) {
        ...
        message.style.color = "red"; // Warnai teks menjadi merah
    } else {
...
function updateCourse() {
    ...
    // Update data kursus
    const oldCredits = courses[editIndex].credits;
    const oldPoints = courses[editIndex].points;

    courses[editIndex] = { subject, credits, grade, gradeScale, points };
